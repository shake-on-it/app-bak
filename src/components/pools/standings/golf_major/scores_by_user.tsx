...
const ScoresByUser = ({ eventDone, payouts, poolEntries, poolEntriesByGolfer, results, users }: Props) => {
  if (!results) {
    return <>results are not yet available</>;
  }

  const calculatePayout = (key: string) => payouts[results[key].rank || ''] / poolEntriesByGolfer[key].length;
